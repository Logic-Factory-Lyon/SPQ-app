@extends('layouts.app')
@section('title', __('app.my_projects_title'))
@section('content')
    <x-page-header title="{{ __('app.my_projects_title') }}" />

    @if($projects->isEmpty())
        <x-empty-state title="{{ __('app.no_project') }}" description="{{ __('app.no_projects_client') }}" />
    @else
            ...
            <a href="{{ route('portal.projects.show', $project) }}"
               class="block bg-gray-900 rounded-xl border border-gray-800 hover:border-indigo-700 p-5 transition-colors">
                ...
                <div class="flex items-center gap-4 text-xs text-gray-500">
                    <span>{{ $project->members_count }} {{ __('app.member_count') }}</span>
                    @if($project->macMachines->isNotEmpty())
                        <span class="flex items-center gap-1">
                            ...
                            Mac Mini {{ $project->macMachines->first()->status === 'online' ? __('app.mac_mini_active') : __('app.mac_mini_inactive') }}
                        </span>
                    @endif
                </div>
            </a>
