...
@section('content')
    <x-page-header title="{{ $macMachine->name }}"
        subtitle="{{ __('app.project_colon', ['name' => $macMachine->project->name]) }} &mdash; {{ __('app.client_colon', ['name' => $macMachine->project->client->name]) }}" />
    ...
    <div class="grid lg:grid-cols-3 gap-4 mb-8">
        ...
        <div class="bg-gray-900 rounded-xl border border-gray-800 p-5">
            <p class="text-sm text-gray-400 mb-2">{{ __('app.status') }}</p>
            <x-badge :color="$statusColor" class="text-sm px-3 py-1">
                {{ ['online' => __('app.status_online'), 'offline' => __('app.status_offline'), 'unknown' => __('app.status_unknown')][$macMachine->status] ?? $macMachine->status }}
            </x-badge>
            @if($macMachine->last_seen_at)
                <p class="text-xs text-gray-600 mt-2">{{ __('app.vu') }} {{ $macMachine->last_seen_at->diffForHumans() }}</p>
            @endif
        </div>
        <x-stat-card label="{{ __('app.pending_messages_label') }}" value="{{ $pendingCount }}" :color="$pendingCount > 0 ? 'yellow' : 'green'" />
        <div class="bg-gray-900 rounded-xl border border-gray-800 p-5">
            <p class="text-sm text-gray-400 mb-2">{{ __('app.agent_profile_short') }}</p>
    ...
    <div class="bg-gray-900 rounded-xl border border-gray-800 p-6 mb-6">
        <div class="flex items-center justify-between mb-3">
            <h3 class="font-semibold text-white">{{ __('app.token_api_daemon') }}</h3>
            <div class="flex items-center gap-3">
                ...
                <a href="{{ route('admin.mac-machines.launcher', $macMachine) }}"
                   class="flex items-center gap-1.5 bg-indigo-600 hover:bg-indigo-500 text-white text-sm font-medium px-4 py-2 rounded-lg transition-colors">
                    ...
                    {{ __('app.download_launcher') }}
                </a>
                <form method="POST" action="{{ route('admin.mac-machines.regenerate-token', $macMachine) }}">
                    @csrf
                    <button type="submit"
                        onclick="return confirm('{{ __("app.regenerate_token_confirm") }}')"
                        class="text-sm text-red-400 hover:text-red-300 transition-colors">
                        {{ __('app.regenerate_token') }}
                    </button>
                </form>
            </div>
        </div>
        ...
        <div class="mt-3 pt-3 border-t border-gray-800 text-xs text-gray-600">
            {{ __('app.download_launcher_help') }} <code class="text-gray-500">.command</code>
        </div>
    </div>
    ...
    <div class="bg-gray-900 rounded-xl border border-gray-800 p-6 mb-6">
        <h3 class="font-semibold text-white mb-3">{{ __('app.daemon_metadata') }}</h3>
        <div class="space-y-2">
            <div class="flex gap-3 text-sm">
                <span class="text-gray-500 w-40 shrink-0">{{ __('app.hostname') }}</span>
            ...
            <div class="flex gap-3 text-sm">
                <span class="text-gray-500 w-40 shrink-0">{{ __('app.daemon_version') }}</span>
                <span class="text-gray-300 font-mono">{{ $macMachine->metadata['openclaw_version'] ?? '—' }}</span>
            </div>
            <div class="flex gap-3 text-sm">
                <span class="text-gray-500 w-40 shrink-0">{{ __('app.detected_agents') }}</span>
                <div>
                    ...
                    @else
                        <span class="text-yellow-500 text-xs">{{ __('app.no_agents_detected') }}</span>
                    @endif
                </div>
            </div>
        </div>
    ...
    <div class="bg-gray-900 rounded-xl border border-gray-800">
        <div class="px-5 py-4 border-b border-gray-800">
            <h3 class="font-semibold text-white">{{ __('app.recent_messages') }}</h3>
        </div>
        <div class="divide-y divide-gray-800">
            @forelse($recentMessages as $msg)
                @php
                    $sc = ['pending' => 'yellow', 'processing' => 'blue', 'done' => 'green', 'error' => 'red'];
                    $sl = ['pending' => __('app.status_pending'), 'processing' => __('app.status_processing'), 'done' => __('app.status_done'), 'error' => __('app.status_error')];
                @endphp
            ...
            @empty
                <p class="text-gray-500 text-sm text-center py-6">{{ __('app.no_recent_messages') }}</p>
            @endforelse
        </div>
    </div>
@endsection
