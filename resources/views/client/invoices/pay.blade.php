@extends('layouts.app')
@section('title', 'Payer la facture ' . $invoice->number)
@section('content')
<x-page-header title="Paiement sécurisé" subtitle="Facture {{ $invoice->number }}" />

<div class="max-w-lg mx-auto">
    <div class="bg-gray-900 rounded-xl border border-gray-800 p-6 mb-6">
        <div class="flex items-center justify-between mb-4">
            <div>
                <p class="text-xs font-semibold text-gray-500 uppercase mb-1">Facture</p>
                <p class="text-white font-mono font-bold text-lg">{{ $invoice->number }}</p>
            </div>
            <div class="text-right">
                <p class="text-xs font-semibold text-gray-500 uppercase mb-1">Montant à payer</p>
    ...
    <div class="bg-gray-900 rounded-xl border border-gray-800 p-6">
        <h3 class="text-white font-semibold mb-4">Informations de carte</h3>

        <form id="payment-form" class="space-y-4">
            <div>
                <label class="block text-sm text-gray-400 mb-1.5">Numéro de carte</label>
                <div id="card-element"
            ...
            <button id="submit-btn" type="submit"
                ...
                <span id="btn-text">Payer {{ number_format($invoice->remaining_balance ?? $invoice->total_ttc, 2, ',', ' ') }} €</span>
...
document.getElementById('payment-form').addEventListener('submit', async (e) => {
    ...
    if (error) {
        ...
        btn.disabled = false;
        btnText.textContent = 'Payer {{ number_format($invoice->remaining_balance ?? $invoice->total_ttc, 2, ',', ' ') }} €';
