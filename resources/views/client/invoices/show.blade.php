...
<div class="grid lg:grid-cols-3 gap-6">
    <div class="lg:col-span-2 space-y-6">
        ...
        <div class="bg-gray-900 rounded-xl border border-gray-800 p-6">
            <div class="grid grid-cols-2 gap-6">
                <div>
                    <p class="text-xs font-semibold text-gray-500 uppercase mb-2">Date d'émission</p>
                    <p class="text-white">{{ $invoice->issue_date?->format('d/m/Y') ?? '—' }}</p>
                </div>
                <div>
                    <p class="text-xs font-semibold text-gray-500 uppercase mb-2">Date d'échéance</p>
                    <p class="{{ $invoice->isOverdue() ? 'text-red-400 font-semibold' : 'text-white' }}">
                        {{ $invoice->due_date?->format('d/m/Y') ?? '—' }}
                        @if($invoice->isOverdue()) <span class="text-xs font-normal">(en retard)</span>@endif
                    </p>
                </div>
                @if($invoice->payment_terms)
                <div class="col-span-2">
                    <p class="text-xs font-semibold text-gray-500 uppercase mb-1">Conditions de paiement</p>
        ...
        <div class="bg-gray-900 rounded-xl border border-gray-800 overflow-hidden">
            <div class="px-5 py-4 border-b border-gray-800">
                <h3 class="font-semibold text-white">Détail des prestations</h3>
            </div>
        ...
        <div class="bg-gray-900 rounded-xl border border-gray-800 overflow-hidden">
            <div class="px-5 py-4 border-b border-gray-800">
                <h3 class="font-semibold text-white">Historique des paiements</h3>
            </div>
            <div class="divide-y divide-gray-800">
                @php $methodLabels = ['stripe'=>'Stripe','bank_transfer'=>'Virement','cheque'=>'Chèque','cash'=>'Espèces','other'=>'Autre']; @endphp
    ...
    <div class="space-y-4">
        <div class="bg-gray-900 rounded-xl border border-gray-800 p-5">
            <p class="text-xs font-semibold text-gray-500 uppercase mb-3">Montant dû</p>
            @if($invoice->status === 'paid')
                <p class="text-2xl font-bold text-green-400">Soldée</p>
            @else
