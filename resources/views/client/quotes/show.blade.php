...
@php
    ...
    $sl = ['draft'=>'En attente','sent'=>'En attente de votre réponse','accepted'=>'Accepté','rejected'=>'Refusé','expired'=>'Expiré'];
@endphp
...
<div class="mb-6 p-5 bg-yellow-900/20 border border-yellow-800 rounded-xl">
    <p class="text-yellow-300 font-semibold mb-1">Ce devis est en attente de votre réponse</p>
    @if($quote->expiry_date)
        <p class="text-yellow-400/70 text-sm mb-4">
            Valable jusqu'au {{ $quote->expiry_date->format('d/m/Y') }}
...
<div class="grid lg:grid-cols-3 gap-6">
    ...
    <div class="space-y-4">
        <div class="bg-gray-900 rounded-xl border border-gray-800 p-5">
            <p class="text-xs font-semibold text-gray-500 uppercase mb-3">Montant total</p>
            <p class="text-3xl font-bold text-white">{{ number_format($quote->total_ttc, 2, ',', ' ') }} €</p>
            <p class="text-xs text-gray-500 mt-1">TTC (dont {{ number_format($quote->total_vat, 2, ',', ' ') }} € TVA)</p>
        </div>
