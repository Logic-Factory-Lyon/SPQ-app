@extends('layouts.app')
@section('title', __('app.new_quote_title'))
@section('content')
<x-page-header title="{{ __('app.new_quote_title') }}" />
...
<form method="POST" action="{{ route('admin.quotes.store') }}"
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div>
                ...
                <textarea name="conditions" rows="3"
                    class="w-full bg-gray-800 border border-gray-700 rounded-lg px-4 py-2.5 text-white focus:outline-none focus:ring-2 focus:ring-indigo-500">{{ old('conditions') }}</textarea>
            </div>
        </div>
    </div>
    ...
    <div class="flex gap-3">
        ...
        <a href="{{ route('admin.quotes.index') }}" class="bg-gray-800 hover:bg-gray-700 text-gray-300 font-semibold px-6 py-2.5 rounded-lg transition-colors">
            {{ __('app.cancel') }}
        </a>
    </div>
</form>
