@extends('layouts.app')
@section('title', __('app.edit') . ' ' . $macMachine->name)
@section('header')
...
@endsection
@section('content')
    <x-page-header title="{{ __('app.edit') }} {{ $macMachine->name }}" />
    <div class="max-w-sm">
        <form method="POST" action="{{ route('admin.mac-machines.update', $macMachine) }}" class="space-y-5">
            ...
            <div class="bg-gray-900 rounded-xl border border-gray-800 p-6 space-y-4">
                <div>
                    <label class="block text-sm text-gray-400 mb-1.5">{{ __('app.machine_name_label') }} *</label>
                ...
                <div>
                    <label class="block text-sm text-gray-400 mb-1.5">{{ __('app.agent_profile_label') }}</label>
                    ...
                    <p class="text-xs text-gray-600 mt-1">{{ __('app.profile_help') }}</p>
            ...
            <div class="flex gap-3">
                ...
                <button type="button" class="bg-red-900/50 hover:bg-red-900 text-red-400 font-semibold px-6 py-2.5 rounded-lg transition-colors ml-auto"
                    onclick="if(confirm('{{ __("app.delete_machine_confirm") }}')) document.getElementById('delete-form-machine').submit()">
                    {{ __('app.delete') }}
                </button>
            </div>
        </form>
