@extends('layouts.app')
@section('title', __('app.new_payment_title') . ' — ' . $invoice->number)
@section('content')
<x-page-header title="{{ __('app.record_payment') }}" subtitle="{{ __('app.invoice_label', ['number' => $invoice->number, 'client' => $invoice->client->name]) }}" />
    ...
    <div class="bg-gray-900 rounded-xl border border-gray-800 p-6">
        <div class="mb-4 p-4 bg-gray-800 rounded-lg">
            <div class="grid grid-cols-2 gap-4 text-sm">
                <div>
                    <p class="text-gray-500">{{ __('app.total_ttc') }}</p>
                    <p class="text-white font-bold">{{ number_format($invoice->total_ttc, 2, ',', ' ') }} €</p>
                </div>
                <div>
                    <p class="text-gray-500">{{ __('app.remaining_balance') }}</p>
        ...
        <div class="space-y-4">
            ...
            <div>
                <label class="block text-sm text-gray-400 mb-1.5">{{ __('app.payment_method') }} *</label>
                <select name="method" required
                    ...
                    <option value="bank_transfer" {{ old('method') === 'bank_transfer' ? 'selected' : '' }}>{{ __('app.method_bank_transfer') }}</option>
                    <option value="stripe" {{ old('method') === 'stripe' ? 'selected' : '' }}>{{ __('app.method_stripe') }}</option>
                    <option value="cheque" {{ old('method') === 'cheque' ? 'selected' : '' }}>{{ __('app.method_cheque') }}</option>
                    <option value="cash" {{ old('method') === 'cash' ? 'selected' : '' }}>{{ __('app.method_cash') }}</option>
                    <option value="other" {{ old('method') === 'other' ? 'selected' : '' }}>{{ __('app.method_other') }}</option>
                </select>
            ...
            <div>
                ...
                <input type="text" name="reference" value="{{ old('reference') }}"
                    placeholder="{{ __('app.reference_placeholder') }}"
                    class="w-full bg-gray-800 border border-gray-700 rounded-lg px-4 py-2.5 text-white focus:outline-none focus:ring-2 focus:ring-indigo-500">
            </div>
