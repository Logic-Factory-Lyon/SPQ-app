...
@section('header')
    <h2 class="text-lg font-semibold text-white">{{ __('app.clients') }}</h2>
@endsection
...
@endsection
@section('content')
    <x-page-header title="{{ __('app.client_accounts') }}" subtitle="{{ __('app.manage_clients') }}" />

    <!-- Search -->
    <form method="GET" class="mb-6 flex gap-3">
        <input type="text" name="search" value="{{ request('search') }}" placeholder="{{ __('app.search_client') }}"
    ...
    @if($clients->isEmpty())
        <x-empty-state title="{{ __('app.no_client') }}" description="{{ __('app.create_first_client') }}"
            action="{{ __('app.new_client') }}" actionUrl="{{ route('admin.clients.create') }}" />
    @else
        <div class="bg-gray-900 rounded-xl border border-gray-800 overflow-hidden">
            <table class="w-full text-sm">
                <thead>
                    <tr class="border-b border-gray-800">
                        <th class="text-left text-xs font-semibold text-gray-500 uppercase tracking-wider px-5 py-3">{{ __('app.company') }}</th>
                        <th class="text-left text-xs font-semibold text-gray-500 uppercase tracking-wider px-5 py-3 hidden md:table-cell">{{ __('app.contact') }}</th>
                        <th class="text-left text-xs font-semibold text-gray-500 uppercase tracking-wider px-5 py-3 hidden lg:table-cell">{{ __('app.projects') }}</th>
                        <th class="text-left text-xs font-semibold text-gray-500 uppercase tracking-wider px-5 py-3 hidden lg:table-cell">{{ __('app.users') }}</th>
                    ...
                    <tr class="hover:bg-gray-800/50 transition-colors">
                        ...
                        <td class="px-5 py-4 text-right">
                            <a href="{{ route('admin.clients.show', $client) }}"
                               class="text-indigo-400 hover:text-indigo-300 text-sm font-medium">{{ __('app.see') }} &rarr;</a>
                        </td>
                    </tr>
                    @endforeach
                </tbody>
