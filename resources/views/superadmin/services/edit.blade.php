@extends('layouts.app')
@section('title', __('app.edit') . ' ' . $service->name)
@section('header')
...
@endsection
@section('content')
    <x-page-header title="{{ __('app.edit') }} {{ $service->name }}" />
    <div class="max-w-lg">
        <form method="POST" action="{{ route('admin.services.update', $service) }}" class="space-y-5">
            ...
            <div class="bg-gray-900 rounded-xl border border-gray-800 p-6 space-y-4">
                ...
                <div>
                    ...
                    <select name="billing_type" required
                        ...
                        <option value="one_time" {{ old('billing_type', $service->billing_type) === 'one_time' ? 'selected' : '' }}>{{ __('app.billing_one_time') }}</option>
                        <option value="monthly" {{ old('billing_type', $service->billing_type) === 'monthly' ? 'selected' : '' }}>{{ __('app.billing_monthly') }}</option>
                        <option value="yearly" {{ old('billing_type', $service->billing_type) === 'yearly' ? 'selected' : '' }}>{{ __('app.billing_yearly') }}</option>
                    </select>
                </div>
            </div>
            <div class="flex gap-3">
                ...
                <button type="button" form="delete-form" class="ml-auto bg-red-900/50 hover:bg-red-900 text-red-400 font-semibold px-6 py-2.5 rounded-lg transition-colors"
                    onclick="if(confirm('{{ __("app.delete_service_confirm") }}')) document.getElementById('delete-form').submit()">
                    {{ __('app.delete') }}
                </button>
            </div>
        </form>
