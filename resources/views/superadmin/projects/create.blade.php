...
@endsection
@section('content')
    <x-page-header title="{{ __('app.new_project_for_client', ['name' => $client->name]) }}" />
    <div class="max-w-lg">
        <form method="POST" action="{{ route('admin.clients.projects.store', $client) }}" class="space-y-5">
            ...
            <div class="bg-gray-900 rounded-xl border border-gray-800 p-6 space-y-4">
                ...
                <div>
                    <label class="block text-sm text-gray-400 mb-1.5">{{ __('app.machine_name') }} *</label>
                    ...
                    <p class="text-xs text-gray-500 mt-1">{{ __('app.api_token_note') }}</p>
                </div>
            </div>
