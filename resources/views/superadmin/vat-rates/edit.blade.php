@extends('layouts.app')
@section('title', 'Modifier ' . $vatRate->name)
@section('header')
...
@endsection
@section('content')
    <x-page-header title="Modifier {{ $vatRate->name }}" />
    <div class="max-w-sm">
        <form method="POST" action="{{ route('admin.vat-rates.update', $vatRate) }}" class="space-y-5">
            ...
            <div class="bg-gray-900 rounded-xl border border-gray-800 p-6 space-y-4">
                ...
                <div class="flex items-center gap-3">
                    ...
                    <label for="active" class="text-sm text-gray-300">Taux actif</label>
                </div>
            </div>
            <div class="flex gap-3">
                ...
                @if(!$vatRate->is_default)
                    <button type="button" class="bg-red-900/50 hover:bg-red-900 text-red-400 font-semibold px-6 py-2.5 rounded-lg transition-colors ml-auto"
                        onclick="if(confirm('Supprimer ce taux de TVA ?')) document.getElementById('delete-form-vat-rate').submit()">
                        Supprimer
                    </button>
                @endif
            </div>
